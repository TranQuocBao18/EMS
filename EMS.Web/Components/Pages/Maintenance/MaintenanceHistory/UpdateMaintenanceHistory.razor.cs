...
{
	public partial class UpdateMaintenanceHistory
	{
		[Parameter]
		public int ID { get; set; }
		public MaintenanceHistoryModel Model { get; set; } = new();
		...
		protected override async Task OnInitializedAsync()
		{
			...
			var res = await ApiClient.GetFromJsonAsync<BaseResponseModel>($"/api/MaintenanceHistory/{ID}");
			if (res != null && res.Success)
			{
				Model = JsonConvert.DeserializeObject<MaintenanceHistoryModel>(res.Data.ToString());
			}
		...
		{
			var res = await ApiClient.PutAsync<BaseResponseModel, MaintenanceHistoryModel>($"/api/MaintenanceHistory/{ID}", Model);
			if (res != null && res.Success)
			{
